      ...
      if (log.isInfoEnabled()) {
        log.info(
            MessageFormat.format(
                "Using base data directory: {0}", DirectoryUtil.getBaseDataDirectory()));
      }
    } catch (Exception e) {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
    ...
    try {
      ...
      if (!(e instanceof IOException)) {
        ...
        // If an unprocessed error comes up from the application layers (Jersey...), rollback the
        // transaction (should not happen)
        if (em.isOpen()) {
    ...
    // No error processing the request : commit / rollback the current transaction depending on the
    // HTTP code
    if (em.isOpen()) {
      if (em.getTransaction() != null && em.getTransaction().isActive()) {
