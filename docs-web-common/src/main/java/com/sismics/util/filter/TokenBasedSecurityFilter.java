...
/**
 * This filter is used to authenticate the user having an active session via an authentication token
 * stored in database. The filter extracts the authentication token stored in a cookie. If the
 * cookie exists and the token is valid, the filter injects a UserPrincipal into a request
 * attribute. If not, the user is anonymous, and the filter injects a AnonymousPrincipal into the
 * request attribute.
 *
 * @author jtremeaux
 */
public class TokenBasedSecurityFilter extends SecurityFilter {
  ...
  private boolean isTokenExpired(AuthenticationToken authenticationToken) {
    ...
    if (authenticationToken.isLongLasted()) {
      ...
      long date =
          authenticationToken.getLastConnectionDate() != null
