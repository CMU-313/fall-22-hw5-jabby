...
public class CommentResource extends BaseResource {
   ...
   */
  @PUT
  public Response add(@FormParam("id") String documentId, @FormParam("content") String content) {
    if (!authenticate()) {
  ...
  public Response get(
      @PathParam("documentId") String documentId, @QueryParam("share") String shareId) {
    authenticate();
    ...
    // Always return OK
    JsonObjectBuilder response = Json.createObjectBuilder().add("comments", comments);
    return Response.ok().entity(response.build()).build();
  }
}
