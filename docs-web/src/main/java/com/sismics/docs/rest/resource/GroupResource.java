...
public class GroupResource extends BaseResource {
   ...
   */
  @PUT
  public Response add(@FormParam("parent") String parentName, @FormParam("name") String name) {
    if (!authenticate()) {
    ...
    if (existingGroup != null) {
      throw new ClientException(
          "GroupAlreadyExists", MessageFormat.format("This group already exists: {0}", name));
    }
    ...
    if (!Strings.isNullOrEmpty(parentName)) {
      ...
      if (parentGroup == null) {
        throw new ClientException(
            "ParentGroupNotFound",
            MessageFormat.format("This group does not exists: {0}", parentName));
      }
      parentId = parentGroup.getId();
    }

    // Create the group
    groupDao.create(new Group().setName(name).setParentId(parentId), principal.getId());
  ...
  public Response update(
    ...
    if (!name.equals(groupName)) {
      String routeModelName =
          RoutingUtil.findRouteModelNameByTargetName(AclTargetType.GROUP, groupName);
    ...
    // Update the group
    groupDao.update(group.setName(name).setParentId(parentId), principal.getId());
  ...
  public Response delete(@PathParam("groupName") String groupName) {
    ...
    if (group.getRoleId() != null) {
      ...
      Set<String> baseFunctionSet =
          roleBaseFunctionDao.findByRoleId(Sets.newHashSet(group.getRoleId()));
  ...
  public Response addMember(
      @PathParam("groupName") String groupName, @FormParam("username") String username) {
    if (!authenticate()) {
      throw new ForbiddenClientException();
    }
    checkBaseFunction(BaseFunction.ADMIN);
    ...
    List<GroupDto> groupDtoList =
        groupDao.findByCriteria(new GroupCriteria().setUserId(user.getId()), null);
    boolean found = false;
  ...
  public Response list(
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("groups", groups);
    return Response.ok().entity(response.build()).build();
  }
  ...
  public Response get(@PathParam("groupName") String groupName) {
    ...
    // Build the response
    JsonObjectBuilder response = Json.createObjectBuilder().add("name", group.getName());
    ...
    List<UserDto> userDtoList =
        userDao.findByCriteria(
            new UserCriteria().setGroupId(group.getId()), new SortCriteria(1, true));
