...
public class AclResource extends BaseResource {
  ...
  @PUT
  public Response add(
      @FormParam("source") String sourceId,
    ...
    AclTargetType type =
        AclTargetType.valueOf(ValidationUtil.validateLength(typeStr, "type", 1, 10, false));
    ...
    if (targetId == null) {
      throw new ClientException(
          "InvalidTarget", MessageFormat.format("This target does not exist: {0}", targetName));
    }
    ...
    // Avoid duplicates
    if (!aclDao.checkPermission(
        acl.getSourceId(), acl.getPerm(), Lists.newArrayList(acl.getTargetId()))) {
  ...
  public Response targetList(@QueryParam("search") String search) {
    ...
    List<UserDto> userDtoList =
        userDao.findByCriteria(new UserCriteria().setSearch(search), sortCriteria);
    for (UserDto userDto : userDtoList) {
      // No need to add users who will skip ACL check anyways
      if (!SecurityUtil.skipAclCheck(Lists.newArrayList(userDto.getId()))) {
        users.add(Json.createObjectBuilder().add("name", userDto.getUsername()));
      }
    }
    ...
    List<GroupDto> groupDtoList =
        groupDao.findByCriteria(new GroupCriteria().setSearch(search), sortCriteria);
    for (GroupDto groupDto : groupDtoList) {
      // No need to add users who will skip ACL check anyways
      if (!SecurityUtil.skipAclCheck(Lists.newArrayList(groupDto.getId()))) {
        groups.add(Json.createObjectBuilder().add("name", groupDto.getName()));
      }
    }
