...
public class DocumentResource extends BaseResource {
  ...
  protected static final DateTimeParser MONTH_PARSER =
      DateTimeFormat.forPattern("yyyy-MM").getParser();
  protected static final DateTimeParser DAY_PARSER =
      DateTimeFormat.forPattern("yyyy-MM-dd").getParser();

  private static final DateTimeFormatter DAY_FORMATTER = new DateTimeFormatter(null, DAY_PARSER);
  private static final DateTimeFormatter MONTH_FORMATTER =
      new DateTimeFormatter(null, MONTH_PARSER);
  ...
  private static final DateTimeParser[] DATE_PARSERS =
      new DateTimeParser[] {YEAR_PARSER, MONTH_PARSER, DAY_PARSER};
  private static final DateTimeFormatter DATE_FORMATTER =
      new DateTimeFormatterBuilder().append(null, DATE_PARSERS).toFormatter();
   ...
   * @apiSuccess {Boolean} route_step.transitionable True if the route step is actionable by the
   *     current user
  ...
  public Response get(
    ...
    DocumentDto documentDto =
        documentDao.getDocument(documentId, PermType.READ, getTargetIdList(shareId));
    if (documentDto == null) {
      throw new NotFoundException();
    }

    JsonObjectBuilder document =
        ...
        outputStream -> {
          try {
            PdfUtil.convertToPdf(
                documentDto, fileList, fitImageToPage, metadata, margin, outputStream);
          } catch (Exception e) {
   ...
   * @apiParam {String} search Search query (see "Document search syntax" on the top of the page for
   *     explanations)
   * @apiParam {Booleans} files If true includes files information
  ...
  public Response list(
    ...
    try {
      AppContext.getInstance()
          .getIndexingHandler()
          .findByCriteria(paginatedList, suggestionList, documentCriteria, sortCriteria);
    } catch (Exception e) {
    ...
    if (Boolean.TRUE == files) {
      Iterable<String> documentsIds =
          CollectionUtils.collect(paginatedList.getResultList(), DocumentDto::getId);
      FileDao fileDao = new FileDao();
    ...
    for (DocumentDto documentDto : paginatedList.getResultList()) {
      ...
      List<TagDto> tagDtoList =
          tagDao.findByCriteria(
              new TagCriteria()
                  ...
                  .setDocumentId(documentDto.getId()),
              new SortCriteria(1, true));
      JsonArrayBuilder tags = Json.createArrayBuilder();
      for (TagDto tagDto : tagDtoList) {
        tags.add(
      ...
      JsonObjectBuilder documentObjectBuilder =
          Json.createObjectBuilder()
              .add("id", documentDto.getId())
      ...
      if (Boolean.TRUE == files) {
        ...
        Collection<File> filesOfDocument =
            CollectionUtils.select(
                filesList, file -> file.getDocumentId().equals(documentDto.getId()));
  ...
  /**
   * Parse a query according to the specified syntax, eg.: tag:assurance tag:other before:2012
   * after:2011-09 shared:yes lang:fra thing
   *
   * @param search Search query
  ...
  private DocumentCriteria parseSearchQuery(String search) {
    ...
    List<TagDto> allTagDtoList =
        tagDao.findByCriteria(new TagCriteria().setTargetIdList(getTargetIdList(null)), null);
    UserDao userDao = new UserDao();
    ...
    for (String criteria : criteriaList) {
      ...
      if (params.length != 2
          || Strings.isNullOrEmpty(params[0])
          || Strings.isNullOrEmpty(params[1])) {
  ...
  public Response add(
    ...
    if (!Constants.SUPPORTED_LANGUAGES.contains(language)) {
      throw new ClientException(
          "ValidationError", MessageFormat.format("{0} is not a supported language", language));
    }
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("id", document.getId());
    return Response.ok().entity(response.build()).build();
  }
  ...
  public Response update(
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("id", id);
    return Response.ok().entity(response.build()).build();
  }
  ...
  public Response importEml(@FormDataParam("file") FormDataBodyPart fileBodyPart) {
    ...
    try {
      ...
      Files.copy(
          fileBodyPart.getValueAs(InputStream.class),
          unencryptedFile,
    ...
    if (tagList != null) {
      ...
      for (String tagId : tagList) {
        if (!tagIdSet.contains(tagId)) {
          throw new ClientException(
              "TagNotFound", MessageFormat.format("Tag not found: {0}", tagId));
        }
        tagSet.add(tagId);
      }
