...
public class RouteModelResource extends BaseResource {
  ...
  public Response list(
    ...
    List<RouteModelDto> routeModelDtoList =
        routeModelDao.findByCriteria(
            new RouteModelCriteria().setTargetIdList(getTargetIdList(null)), sortCriteria);
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("routemodels", routeModels);
  ...
  public Response add(@FormParam("name") String name, @FormParam("steps") String steps) {
    ...
    RouteModelDao routeModelDao = new RouteModelDao();
    String id =
        routeModelDao.create(new RouteModel().setName(name).setSteps(steps), principal.getId());
      ...
      for (int i = 0; i < stepsJson.size(); i++) {
        ...
        try {
          ...
          throw new ClientException(
              "ValidationError", targetTypeStr + " is not a valid ACL target type");
        }
        switch (targetType) {
          case USER:
        ...
        if (type == RouteStepType.VALIDATE) {
          if (transitions.size() != 1) {
            throw new ClientException(
                "ValidationError", "VALIDATE steps should have one transition");
          }
          transitionsNames.add(RouteStepTransition.VALIDATED);
        } else if (type == RouteStepType.APPROVE) {
          if (transitions.size() != 2) {
            throw new ClientException(
                "ValidationError", "APPROVE steps should have two transition");
          }
        ...
        for (int j = 0; j < transitions.size(); j++) {
          ...
          try {
            ...
            throw new ClientException(
                "ValidationError",
                routeStepTransitionStr + " is not a valid route step transition type");
          }
          if (!transitionsNames.contains(routeStepTransition)) {
            throw new ClientException(
                "ValidationError", routeStepTransitionStr + " is not allowed for this step type");
          }
          ...
          for (int k = 0; k < actions.size(); k++) {
            ...
            ValidationUtil.validateRequired(
                routeStepTransitionStr, "step.transitions.actions.type");
            try {
              actionType = ActionType.valueOf(actionTypeStr);
            } catch (IllegalArgumentException e) {
              throw new ClientException(
                  "ValidationError", actionTypeStr + " is not a valid action type");
            }
  ...
  public Response update(
    ...
    routeModelDao.update(routeModel.setName(name).setSteps(steps), principal.getId());
