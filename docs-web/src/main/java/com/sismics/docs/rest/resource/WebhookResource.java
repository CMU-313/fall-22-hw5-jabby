...
public class WebhookResource extends BaseResource {
  ...
  public Response list(@QueryParam("document") String documentId) {
    ...
    List<WebhookDto> webhookDtoList =
        webhookDao.findByCriteria(new WebhookCriteria(), new SortCriteria(2, true));
   ...
   * @apiDescription Each time the specified event is raised, the webhook URL will be POST-ed with
   *     the following JSON payload: {"event": "Event name", "id": "ID of the document or file"}
   * @apiName PutWebhook
   * @apiGroup Webhook
   * @apiParam
   *     {String="DOCUMENT_CREATED","DOCUMENT_UPDATED","DOCUMENT_DELETED","FILE_CREATED","FILE_UPDATED","FILE_DELETED"}
   *     event Event
   ...
   */
  @PUT
  public Response add(@FormParam("event") String eventStr, @FormParam("url") String url) {
    if (!authenticate()) {
      throw new ForbiddenClientException();
    }
    checkBaseFunction(BaseFunction.ADMIN);
    ...
    // Validate input
    WebhookEvent event =
        WebhookEvent.valueOf(ValidationUtil.validateLength(eventStr, "event", 1, 50, false));
    url = ValidationUtil.validateLength(url, "url", 1, 1024, false);

    // Create the webhook
    WebhookDao webhookDao = new WebhookDao();
    webhookDao.create(new Webhook().setUrl(url).setEvent(event));
