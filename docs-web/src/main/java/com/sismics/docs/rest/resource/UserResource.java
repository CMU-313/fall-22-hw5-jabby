...
public class UserResource extends BaseResource {
  ...
  public Response update(
    ...
    if (disabled != null) {
      ...
      if (Constants.GUEST_USER_ID.equals(username)
          || baseFunctionSet.contains(BaseFunction.ADMIN.name())) {
        disabled = false;
      }
  ...
  /**
   * This resource is used to authenticate the user and create a user session. The "session" is only
   * used to identify the user, no other data is stored in the session.
   *
  ...
  public Response login(
    ...
    NewCookie cookie =
        new NewCookie(TokenBasedSecurityFilter.COOKIE_NAME, token, "/", null, null, maxAge, false);
   ...
   * @apiDescription This resource deletes the authentication token created by POST /user/login and
   *     removes the cookie.
  ...
  public Response logout() {
    ...
    try {
      ...
      throw new ServerException(
          "AuthenticationTokenError", "Error deleting the authentication token: " + authToken, e);
    }
    ...
    NewCookie cookie =
        new NewCookie(
            TokenBasedSecurityFilter.COOKIE_NAME,
            null,
            "/",
            null,
            1,
  ...
  public Response delete() {
    ...
    String routeModelName =
        RoutingUtil.findRouteModelNameByTargetName(AclTargetType.USER, principal.getName());
  ...
  public Response delete(@PathParam("username") String username) {
    ...
    Set<String> baseFunctionSet =
        roleBaseFunctionDao.findByRoleId(Sets.newHashSet(user.getRoleId()));
    ...
    // Check that this user is not used in any workflow
    String routeModelName =
        RoutingUtil.findRouteModelNameByTargetName(AclTargetType.USER, username);
    if (routeModelName != null) {
      throw new ClientException("UserUsedInRouteModel", routeModelName);
    }
  ...
  public Response info() {
    ...
    if (!authenticate()) {
      ...
      List<GroupDto> groupDtoList =
          groupDao.findByCriteria(
              new GroupCriteria().setUserId(user.getId()).setRecursive(true), null);
      ...
      response
          .add("base_functions", baseFunctions)
          .add("groups", groups)
          .add(
              "is_default_password",
  ...
  public Response view(@PathParam("username") String username) {
    ...
    List<GroupDto> groupDtoList =
        groupDao.findByCriteria(
            new GroupCriteria().setUserId(user.getId()), new SortCriteria(1, true));
    JsonArrayBuilder groups = Json.createArrayBuilder();
    for (GroupDto groupDto : groupDtoList) {
      groups.add(groupDto.getName());
    }
  ...
  public Response list(
    ...
    List<UserDto> userDtoList =
        userDao.findByCriteria(new UserCriteria().setGroupId(groupId), sortCriteria);
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("users", users);
    return Response.ok().entity(response.build()).build();
  }
   ...
   * @apiDescription This resource lists all active token which can be used to log in to the current
   *     user account.
  ...
  public Response session() {
    ...
    if (!principal.isGuest()) {
      ...
      for (AuthenticationToken authenticationToken :
        ...
        if (authenticationToken.getLastConnectionDate() != null) {
          session.add(
              "last_connection_date", authenticationToken.getLastConnectionDate().getTime());
        }
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("sessions", sessions);
    return Response.ok().entity(response.build()).build();
  }
   ...
   * @apiDescription This resource deletes all active token linked to this account, except the one
   ...
   * @apiDescription Once the onboarding experience has been passed by the user, this resource
   *     prevent it from being displayed again.
   ...
   * @apiDescription This resource enables the Time-based One-time Password authentication. All
   *     following login will need a validation code generated from the given secret seed.
  ...
  public Response enableTotp() {
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("secret", key.getKey());
    return Response.ok().entity(response.build()).build();
  }
  ...
  public Response passwordLost(@FormParam("username") String username) {
    ...
    UserDao userDao = new UserDao();
    List<UserDto> userDtoList =
        userDao.findByCriteria(new UserCriteria().setUserName(username), null);
  ...
  public Response passwordReset(
      @FormParam("key") String passwordResetKey, @FormParam("password") String password) {
    authenticate();
