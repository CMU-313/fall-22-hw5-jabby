...
public class MetadataResource extends BaseResource {
  ...
  public Response list(
    ...
    List<MetadataDto> metadataDtoList =
        metadataDao.findByCriteria(new MetadataCriteria(), sortCriteria);
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("metadata", metadata);
   ...
   */
  @PUT
  public Response add(@FormParam("name") String name, @FormParam("type") String typeStr) {
    if (!authenticate()) {
      throw new ForbiddenClientException();
    }
    ...
    MetadataType type =
        MetadataType.valueOf(ValidationUtil.validateLength(typeStr, "type", 1, 20, false));
