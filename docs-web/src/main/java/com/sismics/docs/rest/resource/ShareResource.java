...
public class ShareResource extends BaseResource {
   ...
   */
  @PUT
  public Response add(@FormParam("id") String documentId, @FormParam("name") String name) {
  ...
  public Response delete(@PathParam("id") String id) {
    ...
    if (!aclDao.checkPermission(acl.getSourceId(), PermType.WRITE, getTargetIdList(null))) {
      throw new ClientException(
          "DocumentNotFound", MessageFormat.format("Document not found: {0}", acl.getSourceId()));
    }
