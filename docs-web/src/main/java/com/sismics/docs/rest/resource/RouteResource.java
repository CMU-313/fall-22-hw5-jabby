...
public class RouteResource extends BaseResource {
  ...
  public Response start(
      @FormParam("routeModelId") String routeModelId, @FormParam("documentId") String documentId) {
    if (!authenticate()) {
    ...
    // Create the route
    Route route = new Route().setDocumentId(documentId).setName(routeModel.getName());
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("route_step", step);
  ...
  public Response validate(
    ...
    DocumentDto documentDto =
        documentDao.getDocument(documentId, PermType.READ, getTargetIdList(null));
    if (documentDto == null) {
      throw new NotFoundException();
    }
    ...
    if (routeStepDto.getType() == RouteStepType.VALIDATE
            && routeStepTransition != RouteStepTransition.VALIDATED
        || routeStepDto.getType() == RouteStepType.APPROVE
            && routeStepTransition != RouteStepTransition.APPROVED
            && routeStepTransition != RouteStepTransition.REJECTED) {
    ...
    routeStepDao.endRouteStep(
        routeStepDto.getId(), routeStepTransition, comment, principal.getId());
    ...
    JsonObjectBuilder response =
        Json.createObjectBuilder()
            .add(
                "readable",
                aclDao.checkPermission(documentId, PermType.READ, getTargetIdList(null)));
    if (newRouteStep != null) {
   ...
   * @apiSuccess {String="APPROVED","REJECTED","VALIDATED"} routes.steps.transition Route step
   *     transition
  ...
  public Response get(@QueryParam("documentId") String documentId) {
    ...
    List<RouteDto> routeDtoList =
        routeDao.findByCriteria(
            new RouteCriteria().setDocumentId(documentId), new SortCriteria(2, false));
    for (RouteDto routeDto : routeDtoList) {
      List<RouteStepDto> routeStepDtoList =
          routeStepDao.findByCriteria(
              new RouteStepCriteria().setRouteId(routeDto.getId()), new SortCriteria(6, true));
    ...
    JsonObjectBuilder json = Json.createObjectBuilder().add("routes", routes);
