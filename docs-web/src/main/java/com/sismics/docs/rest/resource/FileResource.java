...
public class FileResource extends BaseResource {
   ...
   * @apiDescription A file can be added without associated document, and will go in a temporary
   *     storage waiting for one. This resource accepts only multipart/form-data.
   * @apiName PutFile
   * @apiGroup File
  ...
  public Response add(
    ...
    String name =
        fileBodyPart.getContentDisposition() != null
            ? URLDecoder.decode(
                fileBodyPart.getContentDisposition().getFileName(), StandardCharsets.UTF_8)
            : null;
    ...
    try {
      String fileId =
          FileUtil.createFile(
              ...
              unencryptedFile,
              fileSize,
              documentDto == null ? null : documentDto.getLanguage(),
              principal.getId(),
              documentId);

      // Always return OK
      JsonObjectBuilder response =
          Json.createObjectBuilder().add("status", "ok").add("id", fileId).add("size", fileSize);
  ...
  public Response attach(@PathParam("id") String id, @FormParam("id") String documentId) {
    if (!authenticate()) {
      throw new ForbiddenClientException();
    }
    ...
    DocumentDto documentDto =
        documentDao.getDocument(documentId, PermType.WRITE, getTargetIdList(null));
    ...
    // Raise a new file updated event and document updated event (it wasn't sent during file
    // creation)
    try {
      java.nio.file.Path storedFile = DirectoryUtil.getStorageDirectory().resolve(id);
      java.nio.file.Path unencryptedFile =
          EncryptionUtil.decryptFile(storedFile, user.getPrivateKey());
  ...
  @POST
  @Path("{id: [a-z0-9\\-]+}")
  public Response update(@PathParam("id") String id, @FormParam("name") String name) {
    if (!authenticate()) {
      throw new ForbiddenClientException();
    }
  ...
  public Response process(@PathParam("id") String id) {
    ...
    DocumentDto documentDto =
        documentDao.getDocument(file.getDocumentId(), PermType.WRITE, getTargetIdList(null));
    if (documentDto == null) {
      throw new NotFoundException();
    }
  ...
  public Response list(@QueryParam("id") String documentId, @QueryParam("share") String shareId) {
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("files", files);
    ...
    Response.ResponseBuilder builder =
        Response.ok(stream)
            .header(
                ...
                "inline; filename=\"" + file.getFullName("data") + "\"")
  ...
  public Response zip(@QueryParam("id") String documentId, @QueryParam("share") String shareId) {
    authenticate();
  ...
  @Path("zip")
  @Produces({MediaType.APPLICATION_OCTET_STREAM, MediaType.TEXT_PLAIN})
  public Response zip(@FormParam("files") List<String> filesIdsList) {
    authenticate();
        ...
        outputStream -> {
          try (ZipOutputStream zipOutputStream = new ZipOutputStream(outputStream)) {
            ...
            for (File file : fileList) {
              java.nio.file.Path storedfile =
                  DirectoryUtil.getStorageDirectory().resolve(file.getId());
              InputStream fileInputStream = Files.newInputStream(storedfile);
              ...
              try (InputStream decryptedStream =
                  EncryptionUtil.decryptInputStream(fileInputStream, user.getPrivateKey())) {
                ZipEntry zipEntry =
                    new ZipEntry(index + "-" + file.getFullName(Integer.toString(index)));
