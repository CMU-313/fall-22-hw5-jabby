...
public class TagResource extends BaseResource {
  ...
  public Response list() {
    ...
    List<TagDto> tagDtoList =
        tagDao.findByCriteria(
            new TagCriteria().setTargetIdList(getTargetIdList(null)), new SortCriteria(1, true));
    ...
    JsonObjectBuilder response = Json.createObjectBuilder().add("tags", items);
  ...
  public Response get(@PathParam("id") String id) {
    ...
    List<TagDto> tagDtoList =
        tagDao.findByCriteria(
            new TagCriteria().setTargetIdList(getTargetIdList(null)).setId(id), null);
    ...
    if (StringUtils.isEmpty(parentId)) {
      ...
      if (!aclDao.checkPermission(parentId, PermType.READ, getTargetIdList(null))) {
        throw new ClientException(
            "ParentNotFound", MessageFormat.format("Parent not found: {0}", parentId));
      }
    }
