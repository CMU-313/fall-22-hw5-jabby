...
public class TestUserResource extends BaseJerseyTest {
  ...
  public void testUserResource() {
    ...
    JsonObject json =
        target().path("/user").request().acceptLanguage(Locale.US).get(JsonObject.class);
    ...
    Assert.assertEquals("ValidationError", json.getString("type"));
    Assert.assertTrue(
        json.getString("message"), json.getString("message").contains("alphanumeric"));
    ...
    Assert.assertEquals("ValidationError", json.getString("type"));
    Assert.assertTrue(
        json.getString("message"), json.getString("message").contains("must be an email"));
    ...
    target()
        ...
        .queryParam("username", "carol")
        .request()
        .get(JsonObject.class);

    // Check if a username is free : KO
    response =
        target().path("/user/check_username").queryParam("username", "alice").request().get();
    ...
    response =
        target()
            ...
            .post(
                Entity.form(
                    new Form().param("username", " alice ").param("password", " 12345678 ")));
    Assert.assertEquals(Status.OK, Status.fromStatusCode(response.getStatus()));
    ...
    response =
        target()
            ...
            .post(
                Entity.form(
                    new Form().param("username", "intruder").param("password", "12345678")));
    ...
    response =
        target()
            .path("/user/login")
            .request()
            .post(Entity.form(new Form().param("username", "alice").param("password", "error")));
    Assert.assertEquals(Status.FORBIDDEN, Status.fromStatusCode(response.getStatus()));
    ...
    json =
        target()
            ...
            .request()
            .cookie(TokenBasedSecurityFilter.COOKIE_NAME, aliceAuthToken)
            .post(Entity.form(new Form().param("email", " alice2@docs.com ")), JsonObject.class);
    ...
    response =
        target()
            .path("/user/login")
            .request()
            .post(Entity.form(new Form().param("username", "alice").param("password", "12345678")));
    Assert.assertEquals(Status.FORBIDDEN, Status.fromStatusCode(response.getStatus()));
  }
  ...
  public void testUserResourceAdmin() {
    ...
    json =
        target()
            ...
            .cookie(TokenBasedSecurityFilter.COOKIE_NAME, adminToken)
            .post(
                Entity.form(new Form().param("email", "newadminemail@docs.com")), JsonObject.class);
    Assert.assertEquals("ok", json.getString("status"));
  ...
  public void testTotp() {
    ...
    target()
        ...
        .cookie(TokenBasedSecurityFilter.COOKIE_NAME, totp1Token)
        .post(
            Entity.form(new Form().param("code", Integer.toString(validationCode))),
            JsonObject.class);

    // Disable TOTP for totp1
    target()
        .path("/user/disable_totp")
        .request()
        .cookie(TokenBasedSecurityFilter.COOKIE_NAME, totp1Token)
        .post(Entity.form(new Form().param("password", "12345678")), JsonObject.class);
  ...
  public void testResetPassword() throws Exception {
    ...
    Response response =
        target()
            .path("/user/password_lost")
            .request()
            .post(Entity.form(new Form().param("username", "no_such_user")));
    Assert.assertEquals(
        Response.Status.BAD_REQUEST, Response.Status.fromStatusCode(response.getStatus()));
    ...
    json =
        target()
            .path("/user/password_lost")
            .request()
            .post(Entity.form(new Form().param("username", "absent_minded")), JsonObject.class);
    Assert.assertEquals("ok", json.getString("status"));
    ...
    response =
        target()
            ...
            .request()
            .post(
                Entity.form(new Form().param("key", "no_such_key").param("password", "87654321")));
    Assert.assertEquals(
        Response.Status.BAD_REQUEST, Response.Status.fromStatusCode(response.getStatus()));
    json = response.readEntity(JsonObject.class);
    ...
    response =
        target()
            .path("/user/password_reset")
            .request()
            .post(Entity.form(new Form().param("key", key).param("password", " 1 ")));
    Assert.assertEquals(
        Response.Status.BAD_REQUEST, Response.Status.fromStatusCode(response.getStatus()));
    json = response.readEntity(JsonObject.class);
    Assert.assertEquals("ValidationError", json.getString("type"));
    ...
    json =
        target()
            ...
            .request()
            .post(
                Entity.form(new Form().param("key", key).param("password", "87654321")),
                JsonObject.class);
    Assert.assertEquals("ok", json.getString("status"));
    ...
    response =
        target()
            .path("/user/password_reset")
            .request()
            .post(Entity.form(new Form().param("key", key).param("password", "87654321")));
    Assert.assertEquals(
        Response.Status.BAD_REQUEST, Response.Status.fromStatusCode(response.getStatus()));
    json = response.readEntity(JsonObject.class);
    Assert.assertEquals("KeyNotFound", json.getString("type"));
