...
public class TestAppResource extends BaseJerseyTest {
  ...
  public void testInbox() {
    ...
    JsonObject json =
        target()
            ...
            .cookie(TokenBasedSecurityFilter.COOKIE_NAME, adminToken)
            .put(
                Entity.form(new Form().param("name", "Inbox").param("color", "#ff0000")),
                JsonObject.class);
  ...
  public void testLdapAuthentication() throws Exception {
    ...
    //        new LdifFileLoader(directoryService.getAdminSession(), new
    // File(Resources.getResource("test.ldif").getFile()), null).execute();
    //
    //        // Login admin
    //        String adminToken = clientUtil.login("admin", "admin", false);
    //
    ...
    //        Assert.assertEquals("(&(objectclass=inetOrgPerson)(uid=USERNAME))",
