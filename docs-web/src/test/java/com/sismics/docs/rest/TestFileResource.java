...
public class TestFileResource extends BaseJerseyTest {
  ...
  public void testFileResource() throws Exception {
    ...
    target()
        ...
        .request()
        .cookie(TokenBasedSecurityFilter.COOKIE_NAME, file1Token)
        .post(Entity.form(new Form().param("name", "Pale Blue Dot")), JsonObject.class);
  ...
  public void testFileResourceZip() throws Exception {
    ...
    response = target().path("/file/zip").queryParam("id", document1Id).request().get();
    Assert.assertEquals(Status.NOT_FOUND, Status.fromStatusCode(response.getStatus()));
    ...
    // Add a file
    String file2Id =
        clientUtil.addFileToDocument(FILE_EINSTEIN_ROOSEVELT_LETTER_PNG, file1Token, document2Id);
    ...
    response =
        target()
            ...
            .request()
            .cookie(TokenBasedSecurityFilter.COOKIE_NAME, file1Token)
            .post(Entity.form(new Form().param("files", file1Id).param("files", file2Id)));
    Assert.assertEquals(Status.OK, Status.fromStatusCode(response.getStatus()));
    is = (InputStream) response.getEntity();
  ...
  public void testOrphanFile() throws Exception {
    ...
    target()
        .path("/file/" + file1Id + "/attach")
        .request()
        .cookie(TokenBasedSecurityFilter.COOKIE_NAME, fileOrphanToken)
        .post(Entity.form(new Form().param("id", document2Id)), JsonObject.class);
