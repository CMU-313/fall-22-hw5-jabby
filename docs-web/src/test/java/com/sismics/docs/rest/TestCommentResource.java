  ...
  public void testCommentResource() {
    ...
    Response response =
        target()
            ...
            .put(
                Entity.form(
                    new Form().param("id", document1Id).param("content", "Comment by comment2")));
    Assert.assertEquals(Status.NOT_FOUND, Status.fromStatusCode(response.getStatus()));

    // Read comments with comment2 (fail, no read access)
    response =
        target()
            .path("/comment/" + document1Id)
            .request()
            .cookie(TokenBasedSecurityFilter.COOKIE_NAME, comment2Token)
            .get();
    Assert.assertEquals(Status.NOT_FOUND, Status.fromStatusCode(response.getStatus()));
    ...
    json =
        target()
            ...
            .put(
                Entity.form(
                    new Form().param("id", document1Id).param("content", "Comment by comment1")),
                JsonObject.class);
