...
public class AppContext {
  ...
  private void startUp() {
    ...
    try {
      List<Class<? extends IndexingHandler>> indexingHandlerList =
          Lists.newArrayList(
              new ClasspathScanner<IndexingHandler>()
    ...
    if (EnvironmentUtil.isUnitTest()) {
      ...
      ThreadPoolExecutor executor =
          new ThreadPoolExecutor(
              threadCount, threadCount, 1L, TimeUnit.MINUTES, new LinkedBlockingQueue<>());
