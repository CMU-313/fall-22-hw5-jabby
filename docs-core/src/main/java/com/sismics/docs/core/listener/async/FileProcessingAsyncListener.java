...
public class FileProcessingAsyncListener {
  ...
  /**
   * Process a file : Generate thumbnails Extract and save text content
   *
  ...
  /**
   * Extract text content from a file. This is executed outside of a transaction.
   *
   * @param event File event
  ...
  private String extractContent(FileEvent event, User user, File file) {
    ...
    try {
      ...
      if (image != null) {
        ...
        BufferedImage web =
            Scalr.resize(image, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.AUTOMATIC, 1280);
        BufferedImage thumbnail =
            Scalr.resize(image, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.AUTOMATIC, 256);
        image.flush();

        // Write "web" encrypted image
        Path outputFile = DirectoryUtil.getStorageDirectory().resolve(file.getId() + "_web");
        try (OutputStream outputStream =
            new CipherOutputStream(Files.newOutputStream(outputFile), cipher)) {
    ...
    log.info(
        MessageFormat.format(
            "File content extracted in {0}ms: " + file.getId(),
