...
public class RouteStepDao {
  ...
  public RouteStepDto getCurrentStep(String documentId) {
    List<RouteStepDto> routeStepDtoList =
        findByCriteria(
            new RouteStepCriteria().setDocumentId(documentId).setEndDateIsNull(true),
            new SortCriteria(6, true));
  ...
  public List<RouteStepDto> findByCriteria(RouteStepCriteria criteria, SortCriteria sortCriteria) {
    ...
    StringBuilder sb =
        new StringBuilder(
                "select rs.RTP_ID_C, rs.RTP_NAME_C c0, rs.RTP_TYPE_C c1, rs.RTP_TRANSITION_C c2, rs.RTP_COMMENT_C c3, rs.RTP_IDTARGET_C c4, u.USE_USERNAME_C as targetUsername, g.GRP_NAME_C, rs.RTP_ENDDATE_D c5, uv.USE_USERNAME_C as validatorUsername, rs.RTP_IDROUTE_C, rs.RTP_TRANSITIONS_C, rs.RTP_ORDER_N c6")
    ...
    QueryParam queryParam =
        QueryUtil.getSortedQueryParam(new QueryParam(sb.toString(), parameterMap), sortCriteria);
   ...
   */
  public void endRouteStep(
      String id, RouteStepTransition transition, String comment, String validatorUserId) {
    StringBuilder sb = new StringBuilder("update T_ROUTE_STEP r ");
    sb.append(
        " set RTP_ENDDATE_D = :endDate, RTP_TRANSITION_C = :transition, RTP_COMMENT_C = :comment, RTP_IDVALIDATORUSER_C = :validatorUserId ");
