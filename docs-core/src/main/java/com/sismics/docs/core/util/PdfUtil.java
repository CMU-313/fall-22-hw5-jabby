...
public class PdfUtil {
  ...
  public static void convertToPdf(
    ...
    MemoryUsageSetting memUsageSettings =
        MemoryUsageSetting.setupMixed(1000000); // 1MB max memory usage
    memUsageSettings.setTempDir(
        new java.io.File(System.getProperty("java.io.tmpdir"))); // To OS temp
    ...
    try (PDDocument doc = new PDDocument(memUsageSettings)) {
      ...
      if (metadata) {
        ...
        doc.addPage(page);
        try (PdfPage pdfPage =
            new PdfPage(doc, page, margin * Constants.MM_PER_INCH, DocsPDType1Font.HELVETICA, 12)) {
          SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
          pdfPage
              .addText(documentDto.getTitle(), true, DocsPDType1Font.HELVETICA_BOLD, 16)
              .newLine()
              .addText(
                  "Created by "
                      + documentDto.getCreator()
                      + " on "
                      + dateFormat.format(new Date(documentDto.getCreateTimestamp())),
                  true)
              .newLine()
          ...
          pdfPage
              .addText("Language: " + documentDto.getLanguage())
              .newLine()
              .addText(
                  "Files in this document : " + fileList.size(),
                  false,
                  DocsPDType1Font.HELVETICA_BOLD,
                  12);
        }
      }
      ...
      for (File file : fileList) {
        ...
        if (formatHandler != null) {
          formatHandler.appendToPdf(
              unencryptedFile, doc, fitImageToPage, margin, memUsageSettings, closer);
        }
      }
