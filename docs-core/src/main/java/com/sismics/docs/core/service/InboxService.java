...
public class InboxService extends AbstractScheduledService {
    ...
    TransactionUtil.handle(
        () -> {
          ...
          try {
            ...
            try {
              if (inbox != null) {
                // The parameter controls if the messages flagged to be deleted, should actually get
                // deleted.
  ...
  private Folder openInbox() throws Exception {
    ...
    store.connect(
        ConfigUtil.getConfigStringValue(ConfigType.INBOX_USERNAME),
  ...
  private Map<String, String> getAllTags() {
    ...
    TagDao tagDao = new TagDao();
    List<TagDto> tags =
        tagDao.findByCriteria(new TagCriteria().setTargetIdList(null), new SortCriteria(1, true));
