...
public class RouteModelDao {
  ...
  public RouteModel update(RouteModel routeModel, String userId) {
    ...
    // Get the route model
    Query q =
        em.createQuery("select r from RouteModel r where r.id = :id and r.deleteDate is null");
   ...
   * @return List of route models
   */
  public List<RouteModelDto> findByCriteria(
      RouteModelCriteria criteria, SortCriteria sortCriteria) {
    Map<String, Object> parameterMap = new HashMap<String, Object>();
    List<String> criteriaList = new ArrayList<>();

    StringBuilder sb =
        new StringBuilder("select rm.RTM_ID_C c0, rm.RTM_NAME_C c1, rm.RTM_CREATEDATE_D c2");
    ...
    if (criteria.getTargetIdList() != null
      ...
      sb.append(
          " left join T_ACL a on a.ACL_TARGETID_C in (:targetIdList) and a.ACL_SOURCEID_C = rm.RTM_ID_C and a.ACL_PERM_C = 'READ' and a.ACL_DELETEDATE_D is null ");
