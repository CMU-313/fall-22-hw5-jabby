...
public class RelationDao {
  ...
  public List<RelationDto> getByDocumentId(String documentId) {
    ...
    sb.append(
        " join T_DOCUMENT d on d.DOC_ID_C = r.REL_IDDOCFROM_C and r.REL_IDDOCFROM_C != :documentId or d.DOC_ID_C = r.REL_IDDOCTO_C and r.REL_IDDOCTO_C != :documentId ");
  ...
  public void updateRelationList(String documentId, Set<String> documentIdSet) {
    ...
    Query q =
        em.createQuery(
            "select r from Relation r where r.fromDocumentId = :documentId and r.deleteDate is null");
    q.setParameter("documentId", documentId);
    @SuppressWarnings("unchecked")
