...
public class GroupDao {
  ...
  public void delete(String groupId, String userId) {
    ...
    q =
        em.createQuery(
            "update UserGroup ug set ug.deleteDate = :dateNow where ug.groupId = :groupId and ug.deleteDate is not null");
    ...
    q =
        em.createQuery(
            "update Acl a set a.deleteDate = :dateNow where a.targetId = :groupId and a.deleteDate is null");
    ...
    q =
        em.createQuery(
            "update Group g set g.parentId = null where g.parentId = :groupId and g.deleteDate is null");
    q.setParameter("groupId", groupDb.getId());
    q.executeUpdate();
  ...
  public List<GroupDto> findByCriteria(GroupCriteria criteria, SortCriteria sortCriteria) {
    ...
    StringBuilder sb =
        new StringBuilder(
            "select g.GRP_ID_C as c0, g.GRP_NAME_C as c1, g.GRP_IDPARENT_C as c2, gp.GRP_NAME_C as c3, g.GRP_IDROLE_C ");
    ...
    if (criteria.getUserId() != null && criteria.isRecursive()) {
      ...
      for (GroupDto userGroupDto : userGroupDtoList) {
        ...
        findGroupParentHierarchy(
            filteredGroupDtoSet, groupDtoList, userGroupDto, 0); // Indirect groups
      }
   ...
   * @param depth Depth
   */
  private void findGroupParentHierarchy(
      Set<GroupDto> parentGroupDtoSet,
      List<GroupDto> groupDtoList,
      GroupDto userGroupDto,
      int depth) {
    ...
    for (GroupDto groupDto : groupDtoList) {
      if (groupDto.getId().equals(userGroupDto.getParentId())) {
        ...
        findGroupParentHierarchy(
            parentGroupDtoSet, groupDtoList, groupDto, depth + 1); // Find parent's parents
      }
    }
  }
