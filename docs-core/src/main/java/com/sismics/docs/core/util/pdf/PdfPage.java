...
public class PdfPage implements Closeable {
   ...
   */
  public PdfPage(
      PDDocument pdDoc, PDPage pdPage, float margin, PDFont defaultFont, int defaultFontSize)
      throws IOException {
    this.pdPage = pdPage;
  ...
  public PdfPage addText(String text) throws IOException {
    drawText(
        pdPage.getMediaBox().getWidth() - 2 * margin, defaultFont, defaultFontSize, text, false);
    return this;
  }
  ...
  public PdfPage addText(String text, boolean centered) throws IOException {
    drawText(
        pdPage.getMediaBox().getWidth() - 2 * margin, defaultFont, defaultFontSize, text, centered);
    return this;
  }
   ...
   * @throws IOException e
   */
  public PdfPage addText(String text, boolean centered, PDFont font, int fontSize)
      throws IOException {
  ...
  public PdfPage newLine() throws IOException {
    pdContent.newLineAtOffset(
        0,
        -defaultFont.getFontDescriptor().getFontBoundingBox().getHeight() / 1000 * defaultFontSize);
    return this;
  }
  ...
  private void drawText(
      float paragraphWidth, PDFont font, int fontSize, String text, boolean centered)
      throws IOException {
    if (text == null) {
      return;
    }
