...
public class ImageFormatHandler implements FormatHandler {
  ...
  public boolean accept(String mimeType) {
    ...
    return mimeType.equals(MimeType.IMAGE_GIF)
        || mimeType.equals(MimeType.IMAGE_PNG)
        || mimeType.equals(MimeType.IMAGE_JPEG);
  }
  ...
  public void appendToPdf(
    ...
    try (PDPageContentStream contentStream = new PDPageContentStream(doc, page);
      ...
      // Read the image using the correct handler. PDFBox can't do it because it relies wrongly on
      // file extension
      ...
      if (fitImageToPage) {
        ...
        if (widthAvailable / heightAvailable
            < (float) pdImage.getWidth() / (float) pdImage.getHeight()) {
          float imageHeight = widthAvailable / pdImage.getWidth() * pdImage.getHeight();
          contentStream.drawImage(
              pdImage,
              margin * Constants.MM_PER_INCH,
              heightAvailable + margin * Constants.MM_PER_INCH - imageHeight,
              widthAvailable,
              imageHeight);
        } else {
