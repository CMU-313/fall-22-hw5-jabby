...
public class UserDao {
    ...
    if (!Strings.isNullOrEmpty(envBcryptWork)) {
      try {
        ...
        if (envBcryptWorkInt >= 4 && envBcryptWorkInt <= 31) {
          ...
          log.warn(
              Constants.BCRYPT_WORK_ENV
                  + " needs to be in range 4...31. Falling back to "
        ...
        log.warn(
            Constants.BCRYPT_WORK_ENV
                + " needs to be a number in range 4...31. Falling back to "
                + Constants.DEFAULT_BCRYPT_WORK
                + ".");
      }
    }
  ...
  public long getGlobalStorageCurrent() {
    ...
    Query query =
        em.createNativeQuery(
            "select sum(u.USE_STORAGECURRENT_N) from T_USER u where u.USE_DELETEDATE_D is null");
  ...
  public long getActiveUserCount() {
    ...
    DateTime fromDate =
        DateTime.now().minusMonths(1).dayOfMonth().withMinimumValue().withTimeAtStartOfDay();
